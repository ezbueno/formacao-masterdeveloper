...
public class RelatorioService {
	...
	public List<RelatorioItemFaturamento> calcularFaturamentoItens(Integer restauranteId, RelatorioItemFilter filter) {
		...
		if (itemId != 0) {
			 itensObj = pedidoRepository.findItensFaturamento(restauranteId, itemId, dataHoraInicial, dataHoraFinal);
		} else {
			itensObj = pedidoRepository.findItensFaturamento(restauranteId, dataHoraInicial, dataHoraFinal);
		}
